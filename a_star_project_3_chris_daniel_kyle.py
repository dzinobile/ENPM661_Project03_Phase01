...
def A_Star(start_node, goal_node, OL, parent, V, C2C, costsum, step):
    ...
    line_start_xy = (start_node[1][0],start_node[1][1])
    while OL:
        ...
        #line_start_xy = (int(round(fixed_node[0])),int(round(fixed_node[1])))
        
        pygame.draw.line(screen,pallet["blue"],line_start_xy,(int(round(fixed_node[0])),int(round(fixed_node[1]))))
        pygame.display.update()
        line_start_xy = (int(round(child_node[0])),int(round(child_node[1])))
...
while running:
    ...
    alg_state = A_Star(start_node, goal_node, OL, parent, V, C2C, costsum, step=10)
