...
def create_solution_path_video(solution_path, rows, cols, window_size, pallet, C2C, thresh):
    ...
    for node in solution_path:
            pxarray[int(round(node[0])),int(round(node[1]))] = pygame.Color(pallet["blue"])
            pygame.display.update()
        
            frame = pygame.surfarray.array3d(screen)
            frame = np.transpose(frame, (1, 0, 2))  # Pygame surfaces are (width, height), need (height, width)
            solution_frames.append(frame)
        ...
        if(euclidean_distance(fixed_node, goal_node) <= 1.5 ):
            ...
            for child_node, child_cost in actions:
                ...
                if((pxarray[int(child_node_fixed[0]), int(child_node_fixed[1])] == screen.map_rgb(pallet["white"])) or \
                   (pxarray[int(child_node_fixed[0]), int(child_node_fixed[1])] == screen.map_rgb(pallet["green"]))): 
                    continue
                ...
                if(V[child_cost_node] == 0):
                    ...
                    explored.append(child_node_fixed)
...
while running:
    ...
    alg_state, explored, parent, frames, final_goal = A_Star(start_node, goal_node, OL, parent, V, C2C, costsum, step=1, frames=frames, create_video=create_video)
